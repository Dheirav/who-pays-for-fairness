...
\textbf{Not known: \emph{when} it happens.} Their paper never discusses how generous the
...
\textbf{Fourteen populations run \emph{after} the prediction was written down} &
...
points across all fourteen datasets that were run only after this was written down.

\section{What I could not work out}

\textbf{Why the flip happens.} I derived a candidate explanation from theory, wrote the
prediction and its pass mark down in advance, and tested it on fourteen datasets run
...
Loans. Hiring. University admissions.

These are where such fixes are actually deployed, and they are nearly all stingy systems ---
few applicants are approved.

\begin{center}
\fbox{\parbox{0.92\linewidth}{\centering
\textbf{So the normal case in real deployments is the harmful one, \\
and the fairness score never says so.}}}
\end{center}
...
Is ``we found when this happens'' enough on its own for a venue like FAccT or AIES, or does
the paper need ``and here is why'' first?

I can say when the direction flips. I cannot say why --- and the attempt is written up as a
failure rather than left out.
