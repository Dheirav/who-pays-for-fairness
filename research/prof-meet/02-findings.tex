...
\begin{center}
\fbox{\parbox{0.92\textwidth}{\textbf{Update, 23 August.} This deck's numbers are from the
18-population stage and are kept as the record of that stage; \texttt{06-status-report.pdf}
carries the current state. Since this was typeset: 47 populations, a sealed test passed at
9 of 10 on never-measured states, the theory's regime prediction confirmed 27 of 27, and a
second sealed campaign showing the tipping point is population-dependent (0.28--0.65).
\textbf{Further, 25 August:} 57 populations; three adversarial review panels
(twenty-four reviews, no rejects) absorbed; a 2022 anomaly traced to the nominal income
label sliding under inflation and resolved; and the deployable deterministic form of the
mitigation preserves the predicted direction on 10 of 10 populations.}}
\end{center}
...
tested on data the programs had never seen, across 26 runs from 15 populations --- where they report one
...
\subsection{What broke when it was tested harder}

Since the previous version of this document the finding has been attacked four separate
ways, on five populations each rather than one. Two attacks it survived; two found real
limits.
...
\toprule
\textbf{Attack} & \textbf{What it asks} & \textbf{Result} \\
\midrule
A different learner &
  Is this just a property of straight-line models? & \textbf{5 of 5} \\
A looser or tighter rule &
  Is it just an artefact of one setting, 25$\times$ apart? & \textbf{4 of 5} \\
A different way of reaching the same approval rate &
  Is it the approval rate, or how hard the task is? & \textbf{4 of 5} \\
A different fairness definition &
  Does it hold for a rule that never mentions approval rates? & \textbf{Failed} \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{The third attack is the important one.} The original evidence changed the approval
rate by changing what counted as success --- earning over \$20{,}000 against over
\$100{,}000. Those are not the same problem: one is harder than the other. So the evidence
could never tell apart ``the approval rate drives this'' from ``the difficulty drives
this''.

The new test holds the task completely fixed and moves only where the model draws its line.
Same people, same features, same scores. \textbf{The approval rate is what matters}, in four
populations of five, and the two methods agree on \emph{where} the switch happens in three of
five. That also explains something the previous version could only report as a puzzle: the
switch point sits in a different place for mortgages than for income, and it is a genuine
difference between populations rather than an artefact of how the earlier evidence was built.

\textbf{The fourth attack failed, and it is reported as a failure.} Under
\emph{equalized odds} --- a fairness definition concerned with error rates rather than
approval rates --- the relationship drops to $+0.334$ where the original definition gives
$+0.762$ on the very same runs. Adding more populations made it \emph{worse}, not better,
which is what a near-absent effect does. The claim is therefore narrower than it was: it
applies to fairness rules that constrain \emph{how many people are approved}, and not to
fairness rules in general.

\textbf{One population produced no answer at all.} Connecticut's approval rates never fall
below the switch point, so the constraint barely moves anything there and the arithmetic has
nothing to work with. That exposed a flaw in my own method: the original test fixed a bar for
the strength of the relationship but never required that anything actually \emph{move}. I have
added that requirement and re-checked all twenty run-sets scored under the old test.
\textbf{Three become ``no answer''; none of them had ever passed on noise}, so no conclusion
changes --- but the hole was real and it was mine.

\subsection{Where the switch actually sits}

Measured properly --- twelve operating points instead of six, and discarding any arm whose
model is worse than doing nothing --- the switch point is far more stable than I previously
reported:
...
\toprule
\textbf{Population} & \textbf{Field} & \textbf{Switch point} \\
\midrule
...
Four populations, three fields, two countries, four separate data sources --- all between
\textbf{0.511 and 0.576}. An earlier version of this work called the switch point
``population-specific'' on the strength of two much coarser measurements. The practical
instruction is now \textbf{expect about 0.54 and check}, not \emph{measure it from scratch}.

\subsection{The result I did not expect: it belongs to one regime, and the theory names it}

Every result here uses one family of method. So I ran the obvious alternative --- a
\emph{post-processing} method, which adjusts decisions after training instead of changing what
is trained --- across seventeen populations.

\textbf{The relationship vanished.} Correlation $-0.024$, against $+0.585$ for my own method on
the same populations. Taken alone that is the most damaging result in this work.

It is not fragility. The post-processing method \textbf{reads the protected attribute when
deciding}; mine never does. That distinction is exactly the one the theory paper builds on: it
says that when the attribute is available the direction is \textbf{determined} --- the
disadvantaged group gains and the advantaged loses, always --- and only when it is unavailable
is the direction open.

So there is nothing for a predictor to predict in that regime. And their prediction for it
holds in \textbf{18 of 18} of my populations, without exception.

\textbf{What this costs and buys.} Every claim I make is now explicitly about the
attribute-blind case --- which is where nearly every deployed system sits, because setting
different thresholds per group is direct discrimination on its face in most jurisdictions. The
claim is smaller, and it is bounded on exactly the side a reviewer would have attacked first.
...
\textbf{Their conditions hold on none of my 26 runs.} Their theorem is written over the
...
\textbf{r = +0.935}. The two rules agree with each other on 25 of 26 runs. Theirs
...
the finding that their conditions are satisfied on none of my 26 runs.
