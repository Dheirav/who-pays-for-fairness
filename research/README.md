...
| 51 | [The cut is a lottery](docs/51-the-cut-is-a-lottery.md) | What the mitigated mixture actually grants, on all nine deep-tail arms: lift arms are **graded tilts** (probability rises with score below the line), cut arms are **lotteries** — flat keep-probability for everyone above it, exactly zero correlation with score on COMPAS and LSAC. Separates 9/9 where every aggregate failed. Post-fit diagnosis, not prediction |
| 52 | [The campaign answers a different question](docs/52-the-campaign-answers-a-different-question.md) | Both sealed verdicts land as committed: the residual test is **UNDERPOWERED** (3 new crossovers < 6) and the magnitude model **FAILS** (ledger #6). What the 86 arms found instead: the cluster breaks — seven crossovers span **0.28–0.65** — several large states are U-shaped or level up across the whole window, and the natural arms score the 0.54 prior at 5/10 post-hoc, each miss consistent with its own state's sweep. Within-population survives; the universal prior narrows |
| 53 | [Novelty audit for the lottery](docs/53-novelty-audit-for-the-lottery.md) | The lottery claim checked for priority: the specific observation, the tilt-vs-lottery signature and the direction connection are **not found anywhere searched**; the neighbourhood (across-model multiplicity, boundary-randomization theory, derandomization) is mapped with three citations the write-up must carry. Verdict: ours, with fences to build |
| 54 | [The shape seal fails forward](docs/54-the-shape-seal-fails-forward.md) | **FAILS, 4/6 against bar 5, constant not beaten** — but asymmetrically: the 2014 side went 4/4 including two sealed within-state flips (TX, NY), while every 2022 population came out a shape never seen before: **inverted** — positive at low rates, negative at high. The central relationship's first sign-flip, suspects named (RELP shim vs 2022 vintage), NV-2019 diagnostic queued |
| 55 | [Shape is not attribute-independent](docs/55-shape-is-not-attribute-independent.md) | **FAILS, 2/6 against bar 5, constant 6/6.** Every race arm is a classic crossing at 10× the sex arms' signal — same state, same label, same base rate, different attribute, different shape. The label-side mechanism story is refuted by its own seal, and the **gap-size hypothesis** goes on the table, post-hoc — where document 56 kills it before its seal |
| 56 | [The gap hypothesis dies before its seal](docs/56-the-gap-hypothesis-dies-before-its-seal.md) | The pre-seal check kills both forms: natural-point gaps overlap completely between families (medians 0.105 vs 0.106; TX-race classic at 0.066 vs TX-sex anomalous at 0.114), and the anomalous sweeps' arm gaps are healthy. Doc 55's "race gaps far larger" is corrected in place. A gap-*slope* observation is recorded raw and deliberately unsealed; the standing rule gains a corollary — **no same-day seal for a hypothesis born from the campaign it explains**. The shape's governing variable: unidentified, and said plainly |
| 57 | [The inversion was the label sliding](docs/57-the-inversion-was-the-label-sliding.md) | The council's three suspects run to ground: weighting exculpated (PERWT moves 2022 no more than 2018), the recode already acquitted, and the **real-threshold rerun confirms the cause** — at $60k (2018-real-equivalent) Ohio is cleanly classic at its old crossover, AL/SC lose their inverted limbs, NV stays below-guard noise. **No detected time-dependence 2014–2022 at constant real value**; doc 54's "sign-flip" superseded; the caveat becomes a lesson: real-anchor your labels |
| 58 | [The third council, and the audit's own accounting](docs/58-the-third-council-and-the-audits-own-accounting.md) | Ten fresh lenses: 6 minor / 4 major, no rejects. The cheap-compute fixes produce three results: **the audit answers 6 in 10** (45 swept pairs: 27 directional, 6 non-monotone, 11 refusals, all 27 directional verdicts consistent with their natural arms); **the lottery vanishes at natural operating points** (7/7 graded, even Adult at −20%) — a severe-operating-point phenomenon, charge narrowed; **the re-seal survives its own frozen gap floor** (7 of 8 vs constant 5), though IA/NE carried sub-floor gaps. Statistics reframed: paired sign test p ≈ 0.19 beside the binomial 0.046; 9/9 relabelled theory-consistency. All eleven seal→score hash pairs pinned and re-verified |
| 59 | [The deployable form preserves the verdict](docs/59-the-deployable-form-preserves-the-verdict.md) | The examiner's objection answered: thresholding the mixture at 0.5 — the only form a US lender deploys — **agrees with the mixture's direction 10/10** with the parity repair intact (gaps 0.009–0.034 vs 0.08–0.19); rate-matched extraction is impossible in general (probability ties). The domain table is floor-invariant (+0.80..+0.92 at all four floors, provenance verified). Seed stability: 16/19 sealed arms unanimous, and the splits are exactly the near-zero arms — MN (miss) and IA (correct) split 3/5 identically. Two worked traces in the paper |
| 60 | [The boundary does not survive a change of question](docs/60-the-boundary-does-not-survive-a-change-of-question.md) | Sealed cross-task shape test (employment/coverage race arms, seal `d8bfae8` externally stamped): **FAILS 0/4 against bar 3** — every scored call wrong on both sides, constant not beaten. Third informative shape-seal failure after cross-year (54) and cross-attribute (55): the 0.365 boundary is a property of the income question, not of base rates. Direction untouched: every natural arm went down at rates 0.31–0.47 as the rule expects. Population count unchanged (task arms share persons with income arms) |
| 61 | [The night chain hardens three numbers](docs/61-the-night-chain-hardens-three-numbers.md) | Nested rows×seeds bootstrap: COMPAS/SC/OR's published crossovers inside their intervals, **Dutch never brackets (100%)** and is downgraded. The lottery is **in-class necessary**: the two-threshold blind feasible set is empty on all 10 seed-arm pairs, deep-tail grid included. The decile diff shows lift = graded mass into unprivileged deciles 3–8, cut = nothing below decile 9 — the selector question gains its concrete object |
| 62 | [The quantile is the anchor](docs/62-the-quantile-is-the-anchor.md) | The fourth council's cheap batch: **CPI-anchoring and base-rate-anchoring disagree** — at $58,275 (strict CPI) Ohio 2022 stays inverted while at $60k it is classic, so the operative anchor is the label's quantile and doc 57 is refined in place; the lottery's degenerate support observed member-by-member (cut arms carry ~0.87-weight members granting ≤1e-4); no lottery on either HMDA natural arm (9/9 clean); the 0.54 prior recomputed without Dutch changes no call |
| 63 | [Blindness was not load-bearing](docs/63-blindness-was-not-load-bearing.md) | The sealed deconfounding cell (seal `66bc4d5`, stamped): the same reduction with the attribute readable keeps the rate→pool relationship (**r = +0.672**, 13 retained arms) while the theorem's group directions hold **18/18** — so the pool boundary is the **optimizer family, not attribute access**, the −0.024 vanishing belongs to post-processing, the paper's regime framing is corrected in place, and the theory's group-level claim comes out stronger (27/27 + 18/18 across every aware cell) |
| 64 | [The conventions hold their shape](docs/64-the-conventions-hold-their-shape.md) | The economist's three threats measured: design weights shift every crossover **down 0.03–0.08** but FL/PA stay outliers (ordering robust, locations convention-dependent — the 0.54 prior belongs to the unweighted convention); household-clustered intervals widen modestly without dislodging a location; allocation-flagged incomes lift base rates ~+0.03 **uniformly across vintages** — acquitted as a 2022 mechanism, the quantile story stands |
| 65 | [The protocol fails off-market, and the directions hold](docs/65-the-protocol-fails-off-market-and-the-directions-hold.md) | Six-market lending seal: **M2 FAILS 2/6** (the sweep is unreliable on mortgage data — now a six-market sealed result), **M1 UNDERPOWERED** (2 located, both at 0.80–0.85, above even MS+LA); the natural-arm direction strengthens to **7 of 8 markets up**, with Georgia's −0.35 the first above-crossover natural down, sub-guard |
| 66 | [The cohort that refused](docs/66-the-cohort-that-refused.md) | The third cohort: **UNDERPOWERED on all three components** — Brazil's conditional sex gaps (0.005–0.039) sit below the audit's own floor and the gates refused them off-continent, exactly as designed. The label-rarity confound stays open (rule 5/7 vs cutoff-null 6/7 on the survivors, no verdict). Bonus: **MX-2015 brackets the first non-US crossover, 0.423–0.483**. New standing-rule gate: screen gaps between stage B and the runs |
| 67 | [It is the rate](docs/67-it-is-the-rate.md) | The screen-gated race cohort: **the cutoff-only null is beaten 8-9 vs 5** on mixed-route sealed arms, so the rate, not label rarity, carries the prediction — the standing confound resolved in the rule's favor. S1 still FAILS as sealed (the 0.5-prior edged 0.54 by one arm: no in-band privilege for the value), **S2 HOLDS** (classic crossings, ρ 1.0/0.9, effects to −47%), and Brazil adds two more non-US crossovers (0.48–0.54, 0.54–0.60) |
| 68 | [The proxy survives everything](docs/68-the-proxy-survives-everything.md) | The ζ extension: the rate–theory correspondence holds on **150 populations** (r = +0.850, CI +0.813..+0.892), is trim-invariant, survives a gradient-boosted second probe (+0.946), and on the sealed cohorts head-to-head the rate edges the relaxed ordering 18/19 vs 17/19 — the councils' last open analysis item, discharged |
| 69 | [The guard ate the evidence](docs/69-the-guard-ate-the-evidence.md) | The third direction cohort, built for discordance to fix the re-seal's p ≈ 0.19: **S1 FAILS**, 13/14 correct against a constant of 9 (margin +4, CI [+1,+6]) but paired p = 0.062. The rule won **every discordant arm, 4 of 4** — and four of four *is* 0.0625, so the bar was unreachable once the pre-stated magnitude guard removed 10 of 24 arms. The finding is why: corr(|rate−crossover|, |effect|) = **+0.648**, so the guard deletes exactly the near-crossover arms that discriminate. **Guard and power are anti-correlated by construction**, and scaling the cohort scales both |
| 70 | [The rate beats the product](docs/70-the-rate-beats-the-product.md) | The sealed lending cohort, 16 (market × purpose) arms from eight never-downloaded markets: **S1 FAILS** — after the guard every surviving arm went up, so the constant is perfect and cannot be beaten. What it establishes instead: against the **purpose-only null the rate wins 8/8 vs 5/8** (margin +3, CI [+1,+6], p = 0.125), so the loan product does not carry the prediction — document 67's "it is the rate" for the first time on **real allocations** rather than predicted labels |
| 71 | [The prior cannot see the landscape](docs/71-the-prior-cannot-see-the-landscape.md) | Six near-boundary arms swept to locate their own crossovers. **Maryland is explained** (own crossover 0.15–0.29, natural rate 0.508 above it, predicts up, went up) — the rest are not misplaced crossovers but **non-monotone or inverted landscapes**, including the control the rule scored *correct*. So the dead band is not the rule failing: it is that a transported prior **cannot detect a non-monotone landscape**, since detecting one needs the sweep the prior exists to avoid. Free findings: **Missouri 2014 is inverted at $30k**, so inversion is not only a 2022 label artifact; and all three HMDA improvement sweeps are U-shaped, which is the missing mechanism for the sweep failing 2/4 and 2/6. Biased sample by construction — 6 of 107, all chosen for sitting near a boundary |
| 72 | [Every market in the country](docs/72-every-market-in-the-country.md) | All **50 US mortgage markets**, both protected arms, natural arms only (the sweep is unreliable in lending and doc 71 says why). On race arms the direction rule is **12 of 12** where both of the audit's gates are cleared, and 25 of 31 on the parity floor alone — superseding "7 of 8 markets" from one pooled market. And the sex arms are the finding: **0 of 26 clear the 0.05 disparity floor** (median gap 0.013 against race's 0.079, median effect 0.02%), so US mortgage approval has a race gap and essentially no sex gap — the same refusal Brazil produced in the third cohort, now replicated on 26 US states. Post-hoc coverage, not a seal; HMDA 2018 only |
| 73 | [How often does the rule apply at all?](docs/73-how-often-does-the-rule-apply.md) | **Fifty ACS populations drawn at random** (frame and seed committed first), 660 arms, classified by Algorithm 1's own rules. A parity constraint's response admits a directional rule in **78%** of populations and not in 22% (Wilson 95% CI 13–35%); **64% are classic**, 4% inverted. Corrects two things: the project's convenience sample of sweeps gave 15% non-monotone against the honest 22%, and **inversion is not only a 2022 label artifact** — two more sightings at a fixed $50k cutoff in 2018 and 2019. Sex arms are directional 68% against race's 88%, pointing the same way as doc 72's sex-gap refusals. ACS only, one cutoff, n=50 |
| 74 | [What the sweep buys](docs/74-what-the-sweep-buys.md) | The circularity objection, measured rather than asserted: if locating a crossover needs a sweep, why not just fit once at your own operating point? Because one fit returns a point and the sweep returns a neighbourhood. **69 of 639 adjacent arm pairs (11%) disagree on the sign** above the 1.0-point floor, so a single fit's answer does not travel one threshold step. And **11 of 21 disjoint samples (52%) sit within 0.10 of their own crossover** — Florida 2018 is 0.004 away — so for half of them the direction is a threshold the team controls, not a fact about the population, and one fit reports the withdrawal without reporting the lever. Florida also carries two crossovers on the same people (sex 0.284, race 0.439), so there is no constant to look up. The seed-stability third leg is reported and **discounted**: repeated fits at one point would expose it too |
| 75 | [Where the guards came from](docs/75-where-the-guards-came-from.md) | A provenance audit of this project's own thresholds, dated from git. A guard's class belongs to the **(guard, result) pair**, not the guard. Three disclosures: the **2-point void guard is retrospective** — written for the EO pre-registration, imported backwards into the threshold sweep, moving three Connecticut arm sets from refutation to VOID (and the audit checked the other direction: **zero confirmations created**); the **0.10 advisory boundary descends from a refuted clause** that scored 4 of 8 and was withdrawn, kept only in the weaker role where it can refuse but not manufacture; and the **re-seal applies none of them**, scoring all ten raw. Plus 4.3: sweeping the 1.0-point magnitude guard shows **every floor from 0.25 to 5.00 separates** (+13% to +35%, best at 0.25, committed 1.0 gives +34%) — the floor's *existence* is measured, its *value* is operational. Contains a correction to its own first draft, which dated two guards from a constant's name rather than its value |
| 76 | [The instrument cannot test the rule](docs/76-the-instrument-cannot-test-the-rule.md) | **Experiment 7.1, sealed at `4d1909b` before any of its sixteen arms existed, and it FAILS.** HMDA 2021 — a year never touched — split by loan purpose specifically to reach below the crossover, because the 2018 purpose arms span 0.555–0.901. **0 of 8 arms landed below it**, so a constant ties: rule 7/8, constant 7/8; unguarded 10/13 against 10/13. The design error is mine — 2018's span looked like a straddle but only **2 of 9** improvement arms were actually below 0.660, and 2021 then moved the whole instrument up (improvement median 0.765 → 0.824, refinance 0.871 → 0.936). What it establishes is bigger than a pass: US mortgage approval cannot test this rule, now shown **three ways** — state level, fifty-market coverage, and a cohort built to find low arms. Plus the rate beats the purpose-only null in both scorings (7–6, 10–9), and the instrument moved between vintages, so "lending is different" may be a property of a period |
| 77 | [The seal failed and the phenomenon held](docs/77-the-seal-failed-and-the-phenomenon-held.md) | **Experiment 7.1b, sealed at `e5fbd85`. S1 FAILS — and it is the most informative result in the lending record.** Manufactured housing is the one slice of HMDA that reaches below the crossover (0.505 against site-built's 0.850), so pairing both slices per market gives opposite predictions inside one market and a constant cannot tie. The transported 0.660 scores **5 of 8** against a constant's 6 of 8 — fail. But sorted by rate the ten manufactured arms run **`---+++++++`**: monotone, **one sign change**, crossover bracketed at **0.39–0.42**, ρ = +0.624 over rates 0.367–0.885. The prior scores 5 of 10 there; a crossover measured on those arms scores 10 of 10 — **post-hoc and not claimed**, but the single crossing is a property of the arm set however the boundary is drawn. So the first real-allocation arms below any crossover **confirm the phenomenon and refute the transported value**. The prior's record is now three cohorts, one pass |
| 78 | [The crossing was an artefact of ten markets](docs/78-the-crossing-was-an-artefact-of-ten-markets.md) | **Experiment 7.2, sealed at `b60fae9`. FAILS — and it retracts document 77.** Six fresh manufactured markets predicted under both crossovers at once: the located 0.405 scores **1 of 3**, the transported 0.660 scores **3 of 3**. Worse, doc 77's `---+++++++` does **not survive**: sixteen arms give `-+---+--++++++++`, **five sign changes**. The error was mine and specific — monotonicity is a *within-population* claim and doc 77 applied it *across* populations, which the paper itself says does not transport. What survives is stronger than before: **ρ = +0.747, p = 0.001, n = 16**. Over all sixteen, 0.405 calls 13 and 0.660 calls 9 — worse than a constant's 10 — but that is post-hoc and does not rescue the seal. Both sealed misses sit inside 0.405's **dead band** (0.018 and 0.043 away), so the cohort tested the rule exactly where the paper says it fails: a design error, since I wrote a void condition for the previous failure mode and not this one. Five sealed lending attempts, five distinct failures, and no transported crossover has yet predicted direction on real approvals |
| 79 | [The sweep works on one lending slice](docs/79-the-sweep-works-on-one-lending-slice.md) | **Not a seal — Algorithm 1 run on a within-market sweep, and it returns a verdict.** TX 2021 manufactured housing, operating point moved across the frozen grid: pool change crosses zero once, signs `----+`, crossover bracketed **0.448–0.495**, natural rate 0.448 sits below it → **WITHDRAWAL**, and the natural arm agrees. First lending sweep the audit accepts. Corrects doc 71's blanket "sweep unreliable on mortgage data" to "unreliable on the sweeps tried before this". Two honest limits: the natural direction was known first (consistency check, not forecast), and it is one market (not *uniformly* unreliable, not *reliable*). Closes lending: sweep-conditional claim confirmed on one real-allocation market, transported-prior dead across five attempts |
...
**none** of 26 arms — fifteen populations — because the quantity they are stated over diverges on real data, leaving the theorem silent rather than wrong,
