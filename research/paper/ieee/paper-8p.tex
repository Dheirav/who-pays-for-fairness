...
\usepackage[fontsize=7.2pt]{fontsize}
% ---------------------------------------------------------------------------------
...
current model says yes, read against that population's crossover. Across 161 disjoint
...
Brazilian race cohort separates the selection rate from label rarity in the rate's favour, on two populations.
...
161 populations and six domains, and find that a percentile-relaxed form of the
...
against the rate rules' 8--9 --- ten arms from two person-samples, so the confound is
broken within populations rather than across them --- and the selection rate, not label rarity, carries the
...
\item \textbf{A measured scope, and a reason to prefer measuring over transporting.} How
often a parity constraint's response is even \emph{shaped} so that a directional rule can
apply had not been measured. On fifty populations drawn at random --- frame and seed fixed
before any arm ran --- 78\% admit one and 22\% do not, of which 64\% are classic
single-crossing landscapes. That bounds every claim here, and it also explains the
transported prior's worst behaviour: sweeping the arms where a fixed 0.54 fails shows they
are populations with no directional rule to get right, not populations where the rule got it
wrong. A prior read off a dashboard cannot detect that, because detecting it requires the
sweep transporting a prior exists to avoid. The procedure refuses all of them; only the
prior is exposed (Section~\ref{sec:boundaries}).
...
were never randomly chosen; a survey of \textbf{fifty randomly drawn population-label
pairs} spanning 48 disjoint person-samples --- two state-years were drawn under both
protected attributes, which the frame permitted --- with frame and
seed committed before any arm ran, puts the honest figure at \textbf{78\% of populations
admitting a directional rule and 22\% not}, of which 64\% are classic single-crossing
landscapes. Resampling \emph{populations} rather than draws --- two state-years were drawn
under both attributes --- puts the directional share at 66--89\%, and leaving each
population out in turn never moves it below 77\%. The convenience sample was mildly optimistic --- it
implied 15\% --- and the larger number is the one to quote. The refusals are not
