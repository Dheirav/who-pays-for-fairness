...
\usepackage[fontsize=7.4pt]{fontsize}
% ---------------------------------------------------------------------------------
...
current model says yes, read against that population's crossover. Across 107 disjoint
population samples covering six decision domains and eight data sources on five countries, an
attribute-blind parity constraint withdraws favourable decisions when the baseline selection
rate sits below a crossover and extends them when it sits above; on UCI Adult five
in-processing methods each shrink the pool by 7.9--22.1\%, while on mortgage lending the
identical constraint grows it.
...
predicting zero. Crossovers are population-specific --- located values span 0.28 to 0.65 ---
so the fixed 0.54 prior is a fallible substitute for measuring one's own: sealed at
\textbf{9 of 10} on never-measured populations (best constant 6; binomial tail 0.046 against
a guesser, but only $p \approx 0.19$ paired against the constant itself), and 5 of 10 post-hoc
on a second cohort. Of nine sealed or registered direction tests, one passed; we report the
failures because each removed a stronger claim than the one that survives, and because a
cohort built to strengthen that paired statistic showed the attempt is self-defeating --- the
magnitude guard such a test requires deletes the near-crossover arms it needs. A screen-gated
Brazilian race cohort separates the selection rate from label rarity in the rate's favour.
The relationship survives a change of route, a twenty-five-fold tolerance range, a
boosted-tree learner and a protected attribute swapped from sex to race, but disappears under
post-processing --- and a sealed deconfounding test locates that boundary at the optimizer
family, not attribute access. One scope limit belongs here: \textbf{in five of the eight
sources the favourable decisions are predicted labels rather than allocations}, and HMDA, the
one source recording real approvals, is where our sweep protocol failed its held-out test.
The practical outcome is a procedure that locates a team's own crossover from its deployed
model, and refuses when it cannot.
\end{abstract}
...
67 populations and six domains, and find that a percentile-relaxed form of the
...
\emph{arms} --- population-label pairs in the sense of Section~\ref{sec:setup}, not
independent populations, several of which contribute an arm at more than one cutoff ---
with $r = +0.850$ (population-clustered bootstrap 95\% interval $+0.813$ to $+0.892$), is
invariant to the trim percentile, survives a gradient-boosted second probe ($r = +0.946$), and on
...
\textbf{Data.} 107 independent populations across eight data sources.
Two are US surveys: UCI Adult (45{,}222 rows), and ACS Income \cite{ding2021} across forty
US states at 2018, thirty-four further state-years (sixteen at 2014, fourteen at 2019, four
at 2022) and two protected attributes. One records real allocations: HMDA 2018 mortgage
filings for twenty-four states (Mississippi and Louisiana throughout the record; Alabama,
...
arms are informative: Maryland at 0.508 moved up where the prior said down, while Indiana
and Missouri at 0.553 and 0.555 moved down where it said up. A prior that was simply
misplaced would push every miss the same way; these point in both directions, so what the
band exposes is not a shifted crossover but the prior's \emph{lack of resolution} --- which
is what one should expect of a fixed value when located crossovers span 0.28 to 0.65.
Algorithm~\ref{alg:locate} refuses inside a \emph{located} bracket, but a team using the
transported prior without sweeping has no bracket and so met no refusal; on this evidence
the prior should not be read within $\pm 0.05$ of itself. Whether those six sit on the far
side of their own crossovers is untested --- none has a stored sweep --- and it is the
cheapest open experiment here.
...
95\% against 61\%, which is a coin flip. The threshold is doing what it claims.
...
validation, since the natural arm is part of the curve being read. The refusals are not
...
Sealed lending cohort & 8/8 + beat const.\ + paired $p<0.05$; const.\ also 8/8 --- fail &
