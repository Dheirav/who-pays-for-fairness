...
Across 169 disjoint population samples spanning six decision domains, eight data sources and
...
labels rather than allocations actually made. Of ten sealed or registered direction tests, one passed; we report the failures because each removed a stronger claim than the one that
survives.
\end{abstract}
...
the identical constraint grows it by 4.3\% at 0.50. Measured over \textbf{169 disjoint
...
for when the procedure cannot be used at all. Of ten sealed or registered direction tests, one passed; each failure removed a stronger claim than the one that survives, and one cohort
...
\textbf{Data.} 169 independent populations across eight sources. Two are US surveys: UCI Adult
...
and the tail is near $0.09$; read as this paper's own abstract counts them --- ten sealed or registered direction tests --- it is a factor of ten and the tail is $0.46$. Across the
...
committed bar and its outcome, failures included and uncorrected. Counted once and consistently it holds \textbf{nineteen} rows: thirteen fail, three record a
hold (one alongside
a failed first stage), two returned underpowered, and one settled a reading rather than a pass
or a fail. Of the \emph{ten} that test a \emph{direction} rule on populations it had not seen, one
passed. The rows should not be read as nineteen attempts at one claim of which three
...
\begin{table}[!t]
\caption{The eight registered tests that impose a scope condition on the surviving claim. The ledger runs to nineteen rows --- thirteen fail, three hold, two underpowered, one a reading rather than a verdict --- and the complete record carries all of them with their registration and scoring commits. Nothing is withdrawn here; ten rows are elsewhere.}
...
\textbf{Coverage} is eight data sources, five countries and two decades, of which 160 of 169 populations come from two instruments; the eight sources remain the honest count of
...
--- the \textbf{89 natural arms across 73 populations} that clear both of the audit's gates
and on which the plain constraint withdraws. It costs \textbf{0.05 accuracy points} on
average, moves the median exchange rate from 1.33 destroyed per created to 0.94, and brings \textbf{72 of the 89} below one-for-one, none of which were below it already. On the median
arm it does not merely soften the withdrawal but reverses it, from $-2.78\%$ to $+0.88\%$.

Three things that should be said with those numbers rather than after them. The count is over arms and the populations are 73, so the ratio is not 89 independent trials. Sub-1.0 is a
...
\begin{table}[!t]
\caption{What the selection-rate floor buys, on the 89 natural arms over 73 populations that
...
\midrule
Exchange rate (destroyed per created) & 1.33 & \textbf{0.94} \\
Arms at or below one-for-one & 0 of 89 & \textbf{72 of 89} \\
Change in the pool & $-2.78\%$ & $\mathbf{+0.88\%}$ \\
...
that carry them, and the complete nineteen-row ledger. Every figure here is re-derived from
