...
\textbf{Data.} 161 independent populations across eight data sources. Two are US surveys:
UCI Adult (45{,}222 rows) and ACS Income \cite{ding2021} across 102 state-years and two
protected attributes. One records real allocations: HMDA 2018 mortgage filings, 66 rate-bearing
arms over 40 states (30{,}000--305{,}593 records each). The remaining five are IPUMS
International census extracts for Brazil and Mexico, COMPAS \cite{propublica2016},
LSAC bar passage, the Dutch national census of 2001 and Taiwanese credit-card default records
of 2005. A \emph{population} is one such unit --- a state, cohort or census --- and
...
Ratios here are over populations unless they say otherwise.

\textbf{Three survey conventions, because every crossover in this paper depends on them.}
Inherited from the folktables benchmark \cite{ding2021}: ACS person records are used
unweighted, incomes are nominal without the inflation adjustment, and household clustering is
not modelled. So every selection rate here is a property of the unweighted sample rather than
a design-consistent state estimate, and the numeric locations --- including the 0.54 prior
this paper argues about --- are properties of that convention. The weighted re-analysis has
run: read under person weights, \textbf{every located crossover shifts down by 0.03--0.08} in
weighted rate units, without changing any population's ordering or any direction call. That
shift is the same order as the 0.54-against-0.50 distinction of
Section~\ref{sec:resealed}, which is a reason to read the value as a convention-relative
landmark and the \emph{ordering} as the finding. Seed-resampled intervals are
anti-conservative for the same reason, and a household-clustered bootstrap widens them
modestly.

\textbf{What a favourable decision is, and mostly is not.} This belongs here rather than in
Limitations, because it bounds everything that follows. \textbf{In five of the eight sources
no one receives anything}: the favourable decisions are a classifier's labels over people
whose incomes, occupations or bar results already exist, so the pool being given and taken is
a pool of predictions. HMDA is the one source recording real allocative decisions --- and it is
where the direction reverses, and where our sweep protocol failed its held-out test. Whether
the construct transports from status-prediction benchmarks to allocation data is an open
question rather than an assumption we are entitled to.
...
\textbf{It can refuse before it fits anything.} On a task generous enough that the trivial
predictor is already close to the model's accuracy, no deployable classifier can be moved far
enough to cross anything: LSAC refuses at an accuracy floor of 0.890 against a viable band of
0.056, and the test is arithmetic on the baseline, needing no constraint fit at all. Lines
6--8 compute that band and exit before any constraint is
fitted, if no threshold reaches a low selection rate with a model still beating the trivial
predictor. The procedure declines,
...
three others are not, and the complete record states for each whether it was fixed before,
after, or --- in one case, a void guard imported backwards from a later pre-registration that
moved three arm sets from refutation to \textsc{void} --- applied retrospectively in the
conjecture's favour, against which the same audit found no arm set that passed on noise.
...
and are not the same claim. Sweeping the guard over the same forty sealed arms, every value
from 0.25 to 5.00 points separates the reliable arms from the unreliable ones, by between 13
and 35 percentage points, with the largest separation at 0.25 and the committed 1.0 giving
$+34$. So the guard's \emph{existence} is measured and its \emph{value} is operational --- a
round number fixed in a protocol before its arms existed, which the data neither singles out
nor contradicts.
...
rows (measured on a laptop CPU), so a full audit is minutes per population, scaling linearly in the base learner's own fit
time since each reduction call is a sequence of learner refits. On a production
gradient-boosted model over millions of rows that multiplier is the binding cost, and we have
not measured it there. A cheap pre-screen exists: the relaxed-$\zeta$ ordering of
