...
Across 173 disjoint population samples spanning six decision domains, eight data sources and
...
labels rather than allocations actually made. Of eleven sealed or registered direction tests, one passed; we report the failures because each removed a stronger claim than the one that
survives.
\end{abstract}
...
the identical constraint grows it by 4.3\% at 0.50. Measured over \textbf{173 disjoint
...
for when the procedure cannot be used at all. Of eleven sealed or registered direction tests, one passed; each failure removed a stronger claim than the one that survives, and one cohort
...
\textbf{Data.} 173 independent populations across eight sources. Two are US surveys: UCI Adult
...
and the tail is near $0.09$; read as this paper's own abstract counts them --- eleven sealed or registered direction tests --- it is a factor of eleven and the tail is $0.51$. Across the
...
committed bar and its outcome, failures included and uncorrected. Counted once and consistently it holds \textbf{twenty} rows: fourteen fail, three record a
hold (one alongside
a failed first stage), two returned underpowered, and one settled a reading rather than a pass
or a fail. Of the \emph{eleven} that test a \emph{direction} rule on populations it had not seen, one
passed. The rows should not be read as twenty attempts at one claim of which three
...
\begin{table}[!t]
\caption{The eight registered tests that impose a scope condition on the surviving claim. The ledger runs to twenty rows --- fourteen fail, three hold, two underpowered, one a reading rather than a verdict --- and the complete record carries all of them with their registration and scoring commits. Nothing is withdrawn here; ten rows are elsewhere.}
...
dwelling null. \textbf{The structure it describes holds on every arm.} Sorted by selection
rate, the ten manufactured arms run $-,-,-,+,+,+,+,+,+,+$: monotone, one sign change, a
crossover bracketed at 0.39--0.42, on real mortgage approvals spanning 0.367 to 0.885. A
crossover measured on those arms calls all ten, which we report and do not claim --- locating
a boundary from the arms it then scores is the circularity this paper refuses elsewhere. What
is not post-hoc is the single crossing, which is a property of the arm set however the boundary
is drawn. So the first real-allocation arms this work has ever had \emph{below} a crossover
confirm the phenomenon and refute the transported value: 0.660 was located on site-built
purchase and refinance lending, and manufactured housing --- a different product, a different
applicant population --- crosses near 0.405.
...
\textbf{Coverage} is eight data sources, five countries and two decades, of which 164 of 173 populations come from two instruments; the eight sources remain the honest count of
...
--- the \textbf{92 natural arms across 75 populations} that clear both of the audit's gates
and on which the plain constraint withdraws. It costs \textbf{0.05 accuracy points} on
average, moves the median exchange rate from 1.32 destroyed per created to 0.94, and brings \textbf{75 of the 92} below one-for-one, none of which were below it already. On the median
arm it does not merely soften the withdrawal but reverses it, from $-2.83\%$ to $+0.93\%$.

Three things that should be said with those numbers rather than after them. The count is over arms and the populations are 75, so the ratio is not 92 independent trials. Sub-1.0 is a
...
\begin{table}[!t]
\caption{What the selection-rate floor buys, on the 92 natural arms over 75 populations that
...
Arms at or below one-for-one & 0 of 92 & \textbf{75 of 92} \\
Change in the pool & $-2.83\%$ & $\mathbf{+0.93\%}$ \\
...
that carry them, and the complete twenty-row ledger. Every figure here is re-derived from
