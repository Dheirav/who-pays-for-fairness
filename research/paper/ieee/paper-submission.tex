...
are 5\% of the record and are shown for shape, not for scale; Figures~\ref{fig:survey} to~\ref{fig:calibration} carry the population-level evidence.}
\label{fig:crossover}
\end{figure*}
...
the same from one fit; that 24\% over-states the waste, because every sample in the set has
...
\textbf{The result: 78\% admit a directional rule and 22\% do not}, of which 64\% are classic
...
\begin{table}[!t]
\caption{The eight registered tests that impose a scope condition on the surviving claim. The ledger runs to twenty-two rows --- fifteen fail, three hold, two underpowered, one a reading rather than a verdict --- and the complete record carries all of them with their registration and scoring commits. Nothing is withdrawn here; ten rows are elsewhere.}
