...
which its current model says yes. Across 27 independent populations spanning two household
...
that regime the direction is determined, not distributional. The theory's prediction for that regime holds in 18 of 18 populations post-hoc and \textbf{9
of 9} in a pre-registered replication. The practical result is a procedure: locate your own crossover
by sweeping your deployed model's threshold, needing no new data. We also report a
\emph{sealed} prediction on nine populations never previously measured, committed before the
runs, which \textbf{failed at four of eight} --- and the reason is instructive: a refinement
derived from four in-sample populations turned a seven-of-eight prediction into a
four-of-eight one.
\end{abstract}
...
27 populations and six domains, and find its stated conditions --- which are \emph{sufficient,
...
\textbf{Data.} Twenty-seven independent populations across five instruments: UCI Adult (45{,}222
...
populations and the pass mark committed before any of them existed.
...
\subsection{Small populations}\subsection{Small populations}
...
\textbf{The rule has not passed a sealed out-of-sample test.} Section~\ref{sec:sealed}
reports one that failed at four of eight. A simpler form scores seven of eight on the same
arms, but that comparison is post-hoc; the honest position is that the predictive claim is
supported within populations and \emph{not yet} demonstrated on unseen ones.
