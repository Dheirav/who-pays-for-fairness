...
current model says yes. Across 37 independent populations covering six domains and five
instruments, an attribute-blind parity constraint withdraws favourable decisions when the
baseline selection rate sits below a crossover near 0.54, and extends them when it sits
above; on UCI Adult six in-processing methods each shrink the pool by 7.9--22.1\%, while on
mortgage lending the identical constraint grows it. The claim is \emph{sealed} rather than
...
27 of 27 populations, nine pre-registered. The practical outcome is a procedure: a team can
locate its own crossover by sweeping its deployed model's threshold, using no new data.
\end{abstract}
...
in which the protected attribute is unavailable at decision time. Almost every deployed
system sits there, because per-group thresholds are widely read as disparate treatment on
their face \cite{barocas2016,corbett2017}, which rules out the attribute-aware alternatives
in most regulated settings. Under post-processing, which does read the attribute, the effect
...
of 26 for the selection rate. Taken together, this suggests the theory is directionally
right and that what fails is only the form in which its conditions can be checked. The
...
so that no constant could mimic the rule. The bar was committed with the populations before
any arm ran: at least 9 of 10, \emph{and} strictly beating the best constant.
