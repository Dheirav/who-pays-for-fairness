...
Table~\ref{tab:domains} gives the picture. The pre-registered alternative --- ``this is a
...
\begin{enumerate}
\item \textbf{Check the criterion.} The rule is for criteria that constrain selection rates.
\item \textbf{Look up your current approval rate.}
\item \textbf{Locate your crossover.} Sweep your deployed model's threshold, fit the constraint
at each point, and find where the change in the pool turns positive. Discard arms below
$\max(p,1-p)$, check the viable band first, and use ten to twelve points.
\item \textbf{Act.} Below the crossover, expect withdrawal and add a floor on the overall
selection rate: about 0.12 accuracy points, exchange rate $1.47 \rightarrow 0.88$ across
nineteen arms, below 1.0 in sixteen of them.
\item \textbf{Do not forecast the size.}
\end{enumerate}
