...
\textbf{9 of 10} on one cohort of never-measured populations (best constant 6) --- a
...
correction of roughly a factor of two. The pass is evidence, not proof, and the paired
test is why we claim no more. A further limit of
...
--- so the lottery differs in form from the aware-regime optimum. Whether it is
...
quantile to rule out a grid-edge artifact --- \textbf{the feasible set is empty on all
ten seed-arm pairs}: no score-informed blind mixture of that form attains the
lottery's parity gap and pool size at either signature arm. The lottery is therefore
not an optimiser failure a vendor could be expected to avoid; at those operating
points the constraint itself admits no informed blind solution of that size, which is
exactly why the honest charge is the missing announcement, not the randomization. The known
...
finding the direction verdict alone would hide.
...
shows a sign call has nothing to grip: Maryland missed at $0.508$ inside the cluster, and
...
(Section~\ref{sec:scope}) --- in US credit, the self-test privilege of Regulation~B exists for
exactly this. It fits the constraint $12 \times \mathrm{seeds}$ times; with five seeds
...
Sealed cross-task shape & scored$-$1 + beat const.; 0 of 4 --- fail & the base-rate
boundary is a property of the income question: on two new tasks every scored call
inverts, while the direction rule's expectation held at every natural arm \\
\bottomrule
\end{tabular}
\end{table}
...
an outsider must take on trust; the third sealed cohort will anchor its seal externally
(a hosting-provider push receipt or an independent timestamping service) so that the
chronology no longer depends on our clock. A checksum manifest of every input data file
