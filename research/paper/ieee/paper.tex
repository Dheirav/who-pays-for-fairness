...
difference between them is what a sealed test exists to expose. All claims are bounded by
data vintage: four 2022 populations invert the relationship for reasons not yet
identified, so nothing here should be applied to 2020s-era data unmeasured. The relationship survives
...
after the first failed, a sequential context a reader should weight; the adequately
powered third cohort belongs off-instrument and is future work. The prediction used
...
test: the 2022 inversion means no prior fitted here, including 0.54, substitutes for
measuring one's own recent data, and an inverted or non-monotone sweep is a finding about
your system, not a failed audit. Re-run it whenever the model or the population it scores
is refit. We do not claim that the selection rate \emph{causes} the direction; we claim
...
lottery live. Blindness, in these measurements, is not neutrality.
