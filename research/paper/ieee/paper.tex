...
team cannot tell from its own dashboard which it is about to cause. We show the direction is set by the \emph{population} rather than by which in-processing
method is chosen, and is predictable before deployment from the rate at which the current
model says yes, read against that population's own crossover --- the rate a team already has,
the crossover it must measure. Across 179 disjoint population samples covering six decision domains
...
audit that locates a team's own crossover and \emph{refuses} when it cannot: on fifty populations drawn at random, frame and seed fixed in advance, 78\% admit a
directional rule and 22\% do not. That is the monotonicity gate alone; run end to end with every
gate applied, the audit returns a direction on 63 of 104 stored sweeps --- \textbf{a little over six times in ten} --- and hands back the rest as unanswerable rather than scoring them.
...
rate's favour, and a sealed lending cohort beat a loan-purpose null on real approvals. Crossovers are population-specific --- located values span 0.22 to 0.81 --- so a fixed prior is a fallible substitute for measuring one's own: the rule sealed at
...
value 0.54 scored 5 of 10 on a second cohort and holds no privilege over 0.50. Of twelve sealed or registered direction tests, one passed; we report the failures because each
...
it by 4.3\%, at an exchange rate of 0.50. Measured across \textbf{179 disjoint population
samples} spanning six decision domains, eight data sources and five countries. Rates understate this: measured in \emph{people} rather than rates, between 0.66 and
0.74 of the two gap-closing flows are advantaged-group members losing a favourable decision, and the
...
before any arm ran, 78\% admit one and 22\% do not; of those admitting a rule, 64\% are classic
...
fares worse and we no longer present it as a finding. Crossovers are population-specific, located values spanning 0.22 to 0.81; the value failed its own strict bar on the race cohort
(a 0.50 prior edged it by one arm in-band), on a second, post-hoc cohort of larger states it scored 5 of 10 --- each miss agreeing with
that population's own measured crossover --- and on a third, sealed on real mortgage approvals
where the rule could finally be tested below a crossover, 5 of 10 again. Three cohorts, one
pass. It is a fallible substitute for measuring, with a failure rate we can quote, and
...
computable test for when the procedure cannot be used at all. Of twelve sealed or registered direction tests, one passed; we report the failures because each removed a stronger claim
...
dataset} that leaves both groups with fewer favourable classifications, which they read as a
Pareto violation. Their existence claim is prior to ours and we do not repeat it as new; what
they do not do is parameterise the effect, and their conclusion --- that welfare moves
non-monotonically in the fairness tolerance within one population --- is about a different
axis from ours, which compares the unconstrained and constrained models across populations.
Liu \emph{et al.} \cite{liu2018} is the closest structural analogue: a critical selection rate
at which the sign of a parity constraint's effect flips, with the baseline's position relative
to it doing the predicting. Three things differ, and the resemblance is worth stating plainly
rather than leaving for a reader to find: their flipping quantity is one group's expected
score change over a time step, ours is a one-shot headcount summed over both groups; their
axis is the protected group's own rate, ours the model's overall rate; and their policies are
group-specific where ours are blind. Menon and Williamson \cite{menon2018} give the form of
the parity-optimal blind classifier as a per-instance threshold correction, which pushes some
subjects out of the positive set and pulls others in without evaluating the net. And the
infra-marginal line \cite{corbett2017,corbett2023} supplies the vocabulary for what a mechanism
would look like --- what happens at the margin, not away from it --- and a direction flip of
its own, though theirs is one group's share of a fixed budget rather than a pool that can
change size.

\textbf{On not explaining the mechanism.} Our sealed derivation lost to a constant
(Section~\ref{sec:failures}), and we do not claim a mechanism. We should not overstate that
into an absence of any account. For the attribute-\emph{aware} Bayes-optimal case the
...
decision margin --- an infra-marginality statement in the sense of \cite{corbett2023}, and a
property of the population rather than of the method, which is what we measure. We could not
extend it: our constraint is blind, our classifier is a reduction's randomized mixture rather
than a threshold rule, and the effects we measure are far outside a first-order regime. The
honest statement is that we failed to extend a known marginal argument to the blind
randomized case, not that no account exists.

\section{Setup and Method}
\label{sec:setup}

\textbf{Data.} 179 independent populations across eight data sources.
...
Section~\ref{sec:boundaries} gives its later history. These eight populations are 5\% of
the record and are drawn for shape, not scale; Figures~\ref{fig:survey} and~\ref{fig:crossovers} carry the population-level evidence.}
\label{fig:crossover}
\end{figure*}
...
rate threshold can screen for them. No population reaches a rate of 0.55, and 25 of the 47
with a computable rate sit inside the band where located crossovers fall.}
\label{fig:survey}
\end{figure*}
...
0.22 to 0.81, so that cluster was an artefact of which populations had been swept. Florida
2018 appears twice --- read by sex it crosses at 0.284, read by race at 0.439 --- so a
crossover belongs to a (population, attribute) pair.}
...
18, which is a coin flip. What this measures is the guard's \emph{existence}; its value is a
round number fixed in a protocol, and every floor from 0.25 to 5.00 separates.}
...
same instrument the cluster was built from. Located crossovers now span \textbf{0.22 to 0.81} --- 0.22 to 0.65 on the ACS income sweeps, with the law-school and South Carolina mortgage brackets sitting above 0.80 --- which means the location is population-specific and only sometimes near the middle. The same campaign found that a third of the large states have no crossover to
...
The natural-arm route has since been run market by market across the country:
\textbf{66 arms over 40 states}, forty carrying a race arm with a computable selection rate
and twenty-six a sex arm. That is not the fifty states this paper previously claimed --- four
further arms were measured before the test-set denominator was recorded, so they carry a pool
change but no rate to read it against, and the rest were never extracted. On race arms the direction rule is correct on 25 of the 31 arms clearing the parity floor
and on \textbf{12 of 12} clearing both that floor and the one-point magnitude guard.
\textbf{Neither count discriminates, and we report the constant beside them because this
paper fails other cohorts for exactly that reason.} Every arm in both sets sits at a rate of
0.82 or above, well clear of every located lending crossover, so the rule predicts extension
on all of them and a constant ``up'' scores 25 of 31 and 12 of 12 identically. What the
counts establish is that the constraint extends the pool where the rule says it should, on
real approvals, across 40 markets --- consistency, not skill. Testing the rule on lending
would need markets whose rates straddle a crossover, and US mortgage approval does not
supply them: the whole instrument sits high. That supersedes the seven-of-eight this section previously rested
...
prior's want of resolution, exactly as one would expect of a fixed value when located crossovers span 0.22 to 0.81. \textbf{The other five are not misplaced crossovers at all.}
...
discordant calls gives $p \approx 0.19$. And this is not the first sealed attempt at a direction rule, so the binomial tail carries a
sequential correction --- and the size of the family is a judgement we should not make in our
own favour. Read narrowly, as the second attempt after the first failed, the factor is two and
the corrected tail is near 0.09. Read as the paper's own abstract counts them --- twelve sealed or registered direction tests --- the factor is twelve and the tail is 0.56. Either way it is
across the conventional line, and on the wider reading it is not close to it: suggestive, not
significant, under any sequential accounting. The
...
$\approx 0$ against $+0.37$ to $+0.73$). Provenance of the remaining guards, since tuned
constants are where exclusions hide --- and since a guard's provenance is a property of
the \emph{pair} it forms with a result, not of the guard. The 0.40 band span, the 0.05
parity-gap floor and the 12-threshold grid are exploratory, fixed in the threshold sweep's
own pre-registration and confirmatory for every seal after it. Three are not exploratory,
and we state each because a reader should not have to reconstruct it. The \textbf{2-point
void guard} was written for the equalized-odds pre-registration and imported
\emph{backwards} into the threshold sweep once we noticed the earlier test lacked it; doing
so moved three Connecticut arm sets from refutation to \textsc{void}, a change in the
conjecture's favour, and the same audit checked the other direction and found no arm set
that passed on noise --- three refutations removed, zero confirmations created. The \textbf{0.10
advisory boundary} postdates the exploratory sweep it is now applied to, and has a lineage
worth naming: the low-rate observation behind it first appeared as a predictive
\emph{clause}, which was sealed, scored 4 of 8 and was withdrawn
(Section~\ref{sec:sealed}); it survives only in the weaker role of an exclusion, where it
can refuse an arm but cannot manufacture a correct call. The \textbf{2{,}500 noise floor} is
the one this paper's own drafting got wrong: dated from the constant that carries it today
it looks retrospective, and dated from its value it is exploratory, fixed before the
sweeps and frozen in every seal since. Against all of that, the headline
seal is the clean case: the re-seal's 9 of 10 applies \emph{none} of these guards and scores
all ten populations raw, so it has nowhere to hide an exclusion.
...
the protected attribute: audit-time access to $a$, which is legally and practically distinct from decision-time use and is the regime bias-testing provisions contemplate --- available for mortgage, where HMDA compels attribute collection, but restricted under Regulation~B for much other US credit, where a team would face proxy attributes rather than recorded ones (Section~\ref{sec:scope}) --- in US credit, the self-test privilege of Regulation~B
...
\textbf{What the sweep buys that one fit does not.} The cost above invites a direct
objection: a team able to run $12 \times \mathrm{seeds}$ constrained fits could instead run
\emph{one}, at the operating point it actually uses, and read the pool change off it
directly --- so what has prediction bought over measurement? The objection is right that a
single fit answers the question it asks, and right that it is cheaper. It is wrong that the
question a single fit answers is the one a deploying team has, and the difference is
measurable rather than rhetorical. A single fit returns a point; the sweep returns a
neighbourhood. Two measurements over the stored sweeps separate them. First, \textbf{one
fit's answer does not travel}: across 644 adjacent arm pairs whose effects both clear the 1.0-point guard, 70 --- 11\% --- disagree on the \emph{sign}, so the direction a single fit
reports is not reliably a property of the population and the constraint rather than of the
threshold that fit happened to use. Second, and this is what makes the difference a decision
rather than an estimate, \textbf{populations sit close to their own boundary}: of 22 disjoint person samples carrying both a located crossover and a natural arm, 12 sit within 0.10 of that crossover and 7 within 0.05, with Texas manufactured-housing at 0.003 and Florida 2018 at 0.004 the nearest. For those teams
the constraint's direction is not a fact about the population to be accepted or refused ---
it is a threshold they already control, and one fit reports the withdrawal without reporting
the lever. That the crossover cannot simply be looked up instead of measured shows in the
same set: Florida 2018 enters twice, once per protected attribute, crossing at 0.284 read by
sex and 0.439 read by race. The same people, two boundaries.

The limit of that answer belongs with it. The sweep does not buy a better estimate of what
the constraint will do at today's threshold --- repeated fits at one operating point buy
that, and would also expose the one place a single fit is unreliable on its own terms, since
sub-1.0-point arms agree on their sign across seeds only 55\% of the time and the magnitude
guard predicts that about as well as distance from the crossover does. What the sweep buys
is \emph{shape}. Five of the 22 samples sit more than 0.20 from their crossover and would
have reached the same decision from one fit; that 23\% over-states the waste, because every
sample in that set has already passed the monotonicity screen, and the 22\% of populations
admitting no directional rule at all never enter it. A team cannot learn which group it is
in without running the sweep, which is the reply in one line: the audit is not circular,
because its output is not the quantity a single fit measures.

\textbf{Two rates, and they are not the same rate.} The survey's 78\% and the audit's 63 of 104 are different measurements and a reader is entitled to be confused by them meeting on
one page. The 78\% applies \emph{one} gate --- monotonicity --- to fifty populations drawn at
random, and answers ``how often is a population shaped so that a directional rule could
apply''. The 63 of 104 applies \emph{every} gate the algorithm states --- the noise floor, the
two-point void span, monotonicity, and the indeterminate bracket --- to the stored sweeps,
which are a convenience set rather than a random draw, and answers ``how often does the
procedure actually return a direction''. The second is the number a deploying team should
plan against; the first bounds the claim. Neither is the other's denominator, and where the
survey's fifty draws become forty-eight independent samples it is because two state-years were
drawn under both protected attributes --- the same people read twice.

\textbf{What it actually returns, totalled.} Run over the 104 population-label pairs whose stored sweeps resolve to a dataset spec --- fourteen further IPUMS sweeps are stored but unmapped, and are excluded rather than guessed at --- the distribution is: 63 directional verdicts (49 \textsc{withdrawal}, 14 \textsc{extension}), 14 \textsc{non-monotone}, 15 \textsc{void}, eleven refused at the noise floor, and one bracket with no natural arm to read against. Every one of the 63 directional verdicts matches what the fitted constraint did at that population's natural operating
...
The result: \textbf{78\% admit a directional rule and 22\% do not}; of those admitting a rule, 64\% are
...
refusal says the population is too small to audit at all. Counted that way, 92 of the 104 return something a team can act on and twelve return nothing --- eleven sweeps too thin to read and one bracket with no natural arm. The honest summary: the audit gives a \emph{direction}
a little over six times in ten, tells a team the question is moot or the rule inapplicable in most of the remainder, and is silent on twelve of 104. Finally, an ex-ante verdict earns trust only if checked:
...
\cite{mittelstadt2023}. Table~\ref{tab:floor} is what it buys, measured on the branch Algorithm~1 actually sends it
to: the \textbf{95 natural arms across 78 populations} that clear the audit's noise and parity
gates and on which the plain constraint withdraws. It costs \textbf{0.05 accuracy points} on
average, moves the median exchange rate from 1.32 destroyed per created to 0.94, and brings \textbf{78 of 95} below one-for-one, none of which were below it already. On the median arm it
reverses the withdrawal outright, from $-2.78\%$ to $+0.97\%$. Three things belong with those
numbers rather than after them: the count is over arms and the populations are 78, so it is not 95 independent trials; sub-1.0 is a descriptive count of predicted labels and not a
welfare verdict, per the accounting caveat of Section~\ref{sec:whopays}, and the
group-conditional flows are what a weighted reading would use; and earlier versions of this
paper reported the benefit scaling with the damage at $r \approx -0.99$, which we withdraw ---
the benefit is the damage minus the remainder, so the two share a term and the correlation is
arithmetic rather than a finding. And because magnitude does not transfer across populations,
...
included and uncorrected. Counted once and consistently, the table holds \textbf{twenty-two} rows: fifteen fail, three record a hold (one of them alongside a failed first stage), two returned underpowered, and two settled a reading rather than a pass or a fail. Of the \emph{twelve} that test a \emph{direction} rule on populations it had not seen, one
passed. The rows should not be read as twenty-two attempts
at one claim, of which three succeeded: each test targeted a \emph{different} claim, each failure removed a
...
underpow. & the sweep is unreliable on \emph{these} mortgage sweeps, though on manufactured housing it returns a clean single crossing (Sec.~\ref{sec:limitations}); the natural-arm direction holds 12 of 12 on the auditable arms of 40 markets, where a
...
claim: located crossovers span 0.22--0.81 and nothing yet predicts the location; some large
...
clear the audit's noise and parity gates and on which the plain constraint withdraws. Medians
except where stated; every figure recomputed from stored arms by
\texttt{analyse\_floor}.}
\label{tab:floor}
\centering
\small
\begin{tabular}{@{}lcc@{}}
...
\textbf{Of the twelve sealed or registered tests of a direction rule, one passed}; across the
whole ledger of twenty-two, fifteen fail, three hold, two returned underpowered, and two --- the regime-deconfounding test, whose pre-committed two-reading design returned the method reading, and the Texas within-market sweep of Section~\ref{sec:limitations} --- are outcomes rather than passes or fails of ours. Earlier drafts of this paper described
the same table three different ways; the counts above are derived from it and enforced against
it. The first
...
model of it lost to predicting zero. \textbf{The crossover's location is unexplained and wider than it looked}: located values span 0.22--0.81, wider than the 0.28--0.65 this paper previously quoted at both ends, the sealed test of the two candidate
predictors returned underpowered at three locations, and some populations have no crossover
to locate. \textbf{The lending evidence covers one attribute, not two.} Forty markets carry a
rate-bearing arm on each attribute, but every one of the twenty-six sex arms falls below the audit's own
...
rather than a gap in the method, and the audit refuses those arms correctly; so the lending coverage should not be read as forty markets' worth of evidence on both
attributes. \textbf{And US mortgage approval supplies no market whose rate straddles a
crossover, which is now established three ways rather than inferred once.} The state-level
arms sit at 0.82 and above; the fifty-market coverage sits at 0.82 and above; and a sealed
cohort built specifically to reach lower --- a reporting year this work had never touched,
split by loan purpose because purpose is what moves the approval rate --- returned
\textbf{0 of 8 arms below the crossover} and failed for exactly that reason, the rule scoring
7 of 8 against a constant's 7 of 8 (Section~\ref{sec:failures}).
...
that it is reliable there.
 On real approvals the rule
and ``always extend'' are the same rule, so neither lending count discriminates. Testing the
direction claim on allocations needs a register whose accept rates reach below roughly 0.66,
which is a statement about what is publicly available rather than about the rule.
...
The finding a practitioner can use is small and specific, and it carries its own scope: this audit is prospectively validated only on prediction benchmarks, and on real allocative decisions it has one within-market consistency check and no forecast. \textbf{Run the audit of Section~\ref{sec:procedure} on your own current data --- it will locate your crossover, or
