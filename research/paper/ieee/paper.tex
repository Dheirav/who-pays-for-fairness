...
so what remains is an \emph{unattributed distribution shift} between 2019 and 2022 ---
``vintage'' names the calendar, not the mechanism. Three testable candidates are under
diagnosis: the ACS's documented pandemic-era nonresponse bias, whose corrections live in
the sampling weights this paper's convention omits; the fixed nominal \$50{,}000 label
sliding down a post-2021 earnings distribution compressed at the bottom; and sample
composition shift separable by reweighting 2022 rows to earlier covariates. Until one is
confirmed or all excluded, no claim here should be applied to 2020s-era data unmeasured.
