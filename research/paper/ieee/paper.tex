...
current model says yes, read against that population's crossover. Across 67 independent
populations covering six decision domains and eight data sources on five countries, an attribute-blind parity
constraint withdraws favourable decisions when the baseline selection rate sits below a
crossover, and extends them when it sits above; on UCI Adult five in-processing methods each
shrink the pool by 7.9--22.1\%, while on mortgage lending --- one two-state market,
measured three overlapping ways --- the identical constraint grows it.
...
\textbf{9 of 10} on one cohort of never-measured populations (best constant 6; binomial
tail 0.046 against a guesser, but only $p \approx 0.19$ paired against the constant
itself) --- a
cohort whose arms all reach their rates through the income cutoff --- a confound a
screen-gated Brazilian race cohort has since resolved in the rate's favour, the
cutoff-only reading scoring 5 of 10 against the rate rules' 8--9 --- and 5 of 10
...
is a different task each year --- and a CPI-anchored one still drifts when real incomes
grow, so outcome definitions should be anchored to the distribution they cut (a
quantile), and any deployment should
...
under post-processing ($r = -0.024$ against $+0.585$). A sealed deconfounding test then
separated the two things that change at that boundary: giving the \emph{same}
in-processing optimizer the protected attribute leaves the relationship standing
($r = +0.672$, eighteen populations), so the boundary is the method family, not
attribute access --- blindness turns out not to be load-bearing for the pool claim.
Contemporaneous theory's own group-level prediction --- where the attribute is
readable, the disadvantaged group gains and the advantaged loses --- holds in every
attribute-aware cell we have measured: 27 of 27 under post-processing, nine
pre-registered, and 18 of 18 under aware in-processing. The practical outcome is a procedure: when the
...
57 populations and six domains, and find that a percentile-relaxed form of the
theory's ordering agrees with the measured direction on 24 of 26 arms and correlates
with our predictor at $r = +0.935$: the selection rate operationalizes the theory's
quantity. That correspondence has since been stress-tested: it holds across 150
populations ($r = +0.850$, bootstrap 95\% interval $+0.813$ to $+0.892$), is invariant
to the trim percentile, survives a gradient-boosted second probe ($r = +0.946$), and on
the sealed cohorts head-to-head the rate calls 18 of 19 to the relaxed ordering's 17. The exact conditions --- \emph{sufficient, not necessary} --- were certified
on zero of the 26 by our logistic-probe plug-in, whose ratio diverges in the
$\hat\nu$ tail: an estimator pathology, not a property of the conditions, and we
tried no other estimator (Section~\ref{sec:regime}). The direction rule
itself is \emph{sealed}: committed before ten populations this work had never measured, it
scored 9 of 10 against a best constant of 6 (Section~\ref{sec:resealed}) --- with the
confound stated where the score is: every arm of that cohort reaches its rate through the
income cutoff, so the pass does not yet separate the selection rate from label rarity.
The registered deconfounding cohorts have since run: the sex arms were refused
(Brazil's conditional sex gaps sit below the audit's own disparity floor), and the
screen-gated race cohort then ran powered and \textbf{broke the confound in the
rate's favour} --- on mixed-route sealed arms the cutoff-only reading scored 5 of 10
against the rate rules' 8--9, so the selection rate, not label rarity, carries the
prediction --- while the fixed 0.54 \emph{value} failed its own strict bar (a 0.50
prior edged it by one arm in-band): the prior stays fallible and population-specific,
and the within-population claim passed sealed off-instrument. On a second,
...
their own crossover on a deployed model. Three overlapping estimates of the mortgage
crossover agree, all from one two-state market.
...
\textbf{Data.} Sixty-seven independent populations across eight data sources: UCI Adult
(45{,}222 rows); ACS Income \cite{ding2021} across forty US states, ten further
state-years (2014, 2019 and 2022) and two protected attributes; HMDA 2018 mortgage
filings for eight states (Mississippi and Louisiana throughout the record, with
Alabama, South Carolina, Tennessee, Georgia, North Carolina and Ohio added for the
six-market seal; 79{,}104--305{,}593 records each), where the outcome
is the \emph{lender's} approval --- action-taken codes 1 (originated) and 2 (approved,
not accepted) against 3 (denied), with withdrawn, incomplete, purchased and preapproval
records excluded as not being a completed approve-or-deny --- features restricted by
allow-list to what exists when the application is filed, the race arm White vs.\ Black
applicants, the sex arm Male vs.\ Female (joint applications excluded), and loan purposes
as sub-populations; IPUMS International census extracts for Brazil (2000, 2010) and
Mexico (2015, 2020), sex arms, labels set by income quantile with the loader's filter
and 60{,}000-row sealed subsamples per population (Section~\ref{sec:resealed});
...
mechanism, the fourth candidate cleared. A
...
Table~\ref{tab:crossover}, drawn as first published; its upper anchor (Dutch, 0.576) was
later downgraded by the nested bootstrap (Sec.~\ref{sec:boundaries}), leaving 0.511--0.558
standing. Carrying the table's bootstrap intervals widens the band to roughly
0.43--0.58.}
\label{fig:crossover}
\end{figure*}
...
$^\dagger$Downgraded: under the nested rows-by-seeds bootstrap no resample brackets a
crossing for the Dutch census; its row is kept as the record of the original estimate.
The rates are not one economic object: survey rows are predicted-status rates over
sampled persons, lending rows are approval rates conditional on having applied, and
COMPAS inverts a risk score --- comparisons across blocks carry that caveat.}
\label{tab:crossover}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
Dutch census$^\dagger$ & occupation & 0.576 \,(0.572--0.580) & 100\% \\
\midrule
...
constant --- \textbf{it is the rate}. The sealed S1 nonetheless FAILS by its own
...
shows a sign call has nothing to grip: Maryland missed at $0.508$ at the cluster's edge, and
Minnesota's miss was an effect of $-0.04\%$ that flips sign across seeds. The registered
...
\textbf{What it actually returns, totalled.} Run over the 45 population-label pairs with
stored sweeps in this project (populations never swept cannot enter), the distribution
is: 27 directional verdicts (20 \textsc{withdrawal}, 7 \textsc{extension}), 6
\textsc{non-monotone}, 10 \textsc{void}, one refusal at the noise floor (COMPAS), and
one bracket with no natural arm to read against. Every one of the 27 directional
...
validation, since the natural arm is part of the curve being read. The honest summary:
on this project's own data the audit answers six times in ten, and refuses or reports
non-monotonicity the rest --- a procedure that mostly answers, with its refusals
totalled rather than hidden. Finally, an ex-ante verdict earns trust only if checked:
...
natural-arm direction holds 7 of 8; both located midpoints sit at 0.80--0.85 \\
...
to locate. \textbf{Coverage} is eight data sources, five
...
position, not the price level, is the operative anchor. The lesson replaces the caveat:
...
hold. For notice obligations: a flat-lottery model
...
scoring commit, listed here as (sealed $\to$ scored): HMDA sweep \texttt{e23841b} $\to$
...
race cohort \texttt{2f16cda} $\to$ \texttt{e3e3478}, the recent six anchored
externally (an OpenTimestamps receipt committed in \texttt{research/seals/} before any
of its arms ran) --- each seal a committed analyser
whose population lists an isolation guard forbids changing. An honest limit of this
...
from stage~A onward is anchored at sealing time. A checksum manifest of every input data file
